...
{
    const gchar *texto;
    texto = gtk_entry_get_text(GTK_ENTRY(Edt));
...
int main(int argc, char * argv[])
{
 ...
 //   gtk_label_set_selectable(GTK_LABEL(lblTexto), TRUE);
    ...
    gtk_entry_set_text(GTK_ENTRY(EditText),"Digite");
    gtk_editable_set_editable(GTK_EDITABLE(EditText),TRUE);
    gtk_fixed_put(GTK_FIXED(fixed), EditText, 5, 5);
